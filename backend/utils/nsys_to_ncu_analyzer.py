...
class NSysToNCUAnalyzer:
    ...
    def step1_nsys_analysis(
        self, target_command: List[str], profile_name: str = "overview"
    ) -> str:
        """第一步：使用nsys进行全局性能分析"""
        ...
        nsys_cmd = [
            ...
            "profile",
            "-o",
            str(nsys_profile.with_suffix("")),  # nsys会自动添加.nsys-rep
            "-t",
            "cuda,nvtx,osrt",
        ...
        try:
            result = subprocess.run(
                nsys_cmd, capture_output=True, text=True, check=True, env=self.env
            )
    ...
    def step2_extract_hot_kernels(
        self, nsys_file: str, top_k: int = 10, min_duration_ms: float = 0.1
    ) -> List[Dict]:
        ...
        lk_csv = parser.export_kernel_summary_csv(
            nsys_file, self.output_dir / f"{Path(nsys_file).stem}_kernels"
        )
        ...
        if lk_csv:
            ...
            print(
                "⚠️ 未生成 layer_kernels.csv，请确认 NVTX_EVENTS/StringIds/CUPTI 表存在且有 Layer[...] 标签"
            )
        ...
        try:
            ...
            kept = 0
            with (
                open(target_csv, newline="", encoding="utf-8") as f,
                open(run_csv, "w", newline="", encoding="utf-8") as g,
            ):
        ...
        try:
            ...
            hot_json_path.write_text(
                json.dumps(rows_run2, ensure_ascii=False, indent=2), encoding="utf-8"
            )
        ...
        print(
            f"ℹ️ 已按要求使用 Run[2] JSON（{len(self.hot_kernels)} 条），顺序保留且不去重。"
        )
        return self.hot_kernels

    def step3_ncu_targeted_analysis(
        self,
        ...
        if any(
            self._is_placeholder_name(str(k.get("name", "")))
            for k in kernels_to_analyze
        ):
            print(
                "🔍 检测到占位/数字kernel名，触发 ncu --list-kernels 进行真实名称发现..."
            )
            discovered = self.list_kernels_with_ncu(target_command)
            selected = self._select_real_kernels(
                discovered,
                len(kernels_to_analyze) if max_kernels is None else max_kernels,
            )
        ...
        for i, kernel_info in enumerate(kernels_to_analyze):
            ...
            print(
                f"🎯 正在分析kernel {i + 1}/{len(kernels_to_analyze)}: {kernel_name[:60]}..."
            )

            def attempt_profile(
                attempt_cmd: List[str], attempt_tag: str
            ) -> Optional[str]:
                ...
                try:
                    ...
                    if res.returncode != 0:
                        ...
                        print(
                            f"⚠️ 尝试 {attempt_tag} 失败(returncode={res.returncode}): {snippet}"
                        )
                        return None
    ...
    def step3_ncu_global_focus(
        self,
        target_command: List[str],
        hot_kernels: List[Dict],
        top_focus: int = 5,
        set_name: str = "compute",
        launch_limit: Optional[int] = None,
        ...
        if not hot_kernels:
            ...
            return None, {}
        # 运行一次全量采集
        full_rep = self.full_ncu_capture(
            target_command,
            profile_name="ncu_full_capture_global",
            set_name=set_name,
        ...
        def _match_entries(target: str) -> List[Any]:
            ...
            matched = [
                km
                for km in metrics_list
                if t_low in km.name.lower() or km.name.lower() in t_low
            ]
            ...
            if not matched:
                ...
                tokens = [
                    tok for tok in re.split(r"[^A-Za-z0-9_]+", t_low) if len(tok) >= 5
                ]
                if tokens:
                    for tok in tokens:
        ...
        for hk in focus_targets:
            ...
            def add_bottleneck(cond: bool, desc: str, severity: str):
                if cond:
                    bottlenecks.append(
                        {"type": "heuristic", "severity": severity, "description": desc}
                    )

            add_bottleneck(sm_eff is not None and sm_eff < 40, "SM效率偏低", "high")
            add_bottleneck(
                dram is not None and dram < 150, "内存带宽可能受限", "medium"
            )
            add_bottleneck(occ is not None and occ < 25, "Occupancy较低", "medium")
            add_bottleneck(
                warp_eff is not None and warp_eff < 70, "Warp执行效率一般", "low"
            )
            focus[kname] = {
    ...
    def full_ncu_capture(
        self,
        target_command: List[str],
        profile_name: str = "ncu_full_capture",
        set_name: str = "compute",
        launch_limit: Optional[int] = None,
        timeout: int = 1200,
    ) -> Optional[str]:
    ...
    def list_kernels_with_ncu(self, target_command: List[str]) -> List[str]:
        ...
        try:
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=600, env=self.env
            )
        ...
        pattern = re.compile(
            r"(Kernel|cuda|cutlass|flash|aten|cublas|gemm|matmul|triton)", re.IGNORECASE
        )
        for line in lines:
    ...
    def _select_real_kernels(
        self, discovered: List[str], max_kernels: int
    ) -> List[str]:
    ...
    def _export_ncu_to_csv(self, ncu_file: str) -> Optional[str]:
        ...
        export_cmd = ["ncu", "--csv", "--log-file", csv_file, "--import", ncu_file]
    ...
    def step4_comprehensive_analysis(
        self, ncu_files: List[str], focus_metrics: Optional[Dict[str, Dict]] = None
    ) -> Dict:
        """第四步：综合分析nsys和ncu结果"""
        ...
        with open(results_file, "w", encoding="utf-8") as f:
            json.dump(
                comprehensive_results, f, indent=2, ensure_ascii=False, default=str
            )
    ...
    def generate_final_report(self, comprehensive_results: Dict) -> str:
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            f.write("集成性能分析报告\n\n")
            f.write(
                f"生成时间: {datetime.now().astimezone().strftime('%Y年%m月%d日')}\n\n"
            )
            ...
            if "kernel_analysis" in nsys_overview:
                ...
                f.write(
                    f" 总kernel执行时间: {kernel_stats.get('total_kernel_time', 0):.2f} ms\n"
                )
            ...
            for i, kernel in enumerate(self.hot_kernels, 1):
                ...
                percent = (
                    (dur / total_kernel_time_ms * 100.0)
                    if total_kernel_time_ms > 0
                    else 0.0
                )
            ...
            for i, (idx, kernel_name, analysis) in enumerate(items, 1):
                ...
                if gu:
                    f.write(
                        f"   - 平均SM效率: {gu.get('average_sm_efficiency', 'N/A')}\n"
                    )
                    f.write(f"   - 最高SM效率: {gu.get('max_sm_efficiency', 'N/A')}\n")
                    f.write(f"   - 最低SM效率: {gu.get('min_sm_efficiency', 'N/A')}\n")
                    f.write(
                        f"   - 低于50%数量: {gu.get('kernels_below_50_percent', 0)} / {gu.get('total_kernels', 0)}\n"
                    )
                # 内存分析
                ...
                if bw:
                    f.write(
                        f"   - 平均带宽: {bw.get('average_bandwidth', 'N/A')} GB/s\n"
                    )
                ...
                if l2:
                    f.write(
                        f"   - 平均L2命中率: {l2.get('average_l2_hit_rate', 'N/A')}\n"
                    )
                    f.write(
                        f"   - 低L2命中率kernel数: {l2.get('kernels_low_l2_hit_rate', 0)}\n"
                    )
                l1 = mem.get("l1_cache_stats", {})
                if l1:
                    f.write(
                        f"   - 平均L1命中率: {l1.get('average_l1_hit_rate', 'N/A')}\n"
                    )
                    f.write(
                        f"   - 低L1命中率kernel数: {l1.get('kernels_low_l1_hit_rate', 0)}\n"
                    )
                # 瓶颈
            ...
            if focus_analysis:
                ...
                for kname, fan in focus_analysis.items():
                    ...
                    if mem:
                        f.write(
                            f"- 平均带宽: {mem.get('average_bandwidth', 'N/A')} GB/s\n"
                        )
...
def create_sglang_analysis_workflow():
    ...
    def run_sglang_integrated_analysis(
        ...
        print(
            f"[DEBUG] run_sglang_integrated_analysis entered: bs={batch_size}, in={input_len}, out={output_len}"
        )
...
def main():
    ...
    parser.add_argument(
        "--sglang-input-len", type=int, default=128, help="SGlang输入长度"
    )
    parser.add_argument(
        "--sglang-output-len", type=int, default=1, help="SGlang输出长度"
    )
    # 兼容：保留未知参数但不使用
